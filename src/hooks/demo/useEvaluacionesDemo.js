  ...
  return useDemoQuery({
    ...
    queryFn: async () => {
      const evaluaciones = mockData.evaluaciones
        ...
        .map(e => {
          const docente = mockData.trabajadores.find(t => t.id === e.docenteId);
          return {
            ...e,
  ...
  return useDemoQuery({
    ...
    queryFn: async () => {
      ...
      let evaluaciones = mockData.evaluaciones
        .filter(e => estudiantesAula.includes(e.estudianteId));
      ...
      return evaluaciones.map(e => {
        const estudiante = mockData.estudiantes.find(est => est.id === e.estudianteId);
        return {
          ...e,
...
export const useBoletinNotasDemo = (estudianteId, periodo) => {
  return useDemoQuery({
    ...
    queryFn: async () => {
      const evaluaciones = mockData.evaluaciones
        .filter(e => e.estudianteId === estudianteId && e.periodo === periodo);
      ...
      const boletinData = Object.entries(materias).map(([materia, evaluacionesMateria]) => {
        const notas = evaluacionesMateria.map(e => parseFloat(e.calificacion)).filter(n => !isNaN(n));
        const promedio = notas.length > 0 ? (notas.reduce((a, b) => a + b, 0) / notas.length).toFixed(1) : '0.0';
      ...
      const promedioGeneral = promedios.length > 0 ? 
        (promedios.reduce((a, b) => a + b, 0) / promedios.length).toFixed(1) : '0.0';
  ...
  return useDemoQuery({
    ...
    queryFn: async () => {
      ...
      if (filtros.docenteId) {
        evaluaciones = evaluaciones.filter(e => e.docenteId === filtros.docenteId);
      }
      ...
      const promediosPorMateria = Object.entries(porMateria).map(([materia, data]) => ({
        materia,
