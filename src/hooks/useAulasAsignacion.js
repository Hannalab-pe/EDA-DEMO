...
export const useAulasAsignacion = (
  ...
  const {
    ...
    queryFn: () => {
      ...
      return soloSinAsignacion
        ? demoAsignacionAulaService.getAulasSinAsignar()
        : demoAulaService.getAllAulas();
    },
  ...
  const {
    ...
    queryFn: () => demoAsignacionAulaService.getAllAsignaciones(),
    staleTime: 5 * 60 * 1000, // 5 minutos
    cacheTime: 10 * 60 * 1000, // 10 minutos
    onSuccess: (data) => {
  ...
  const createAsignacionMutation = useMutation({
    mutationFn: (asignacionData) =>
      demoAsignacionAulaService.createAsignacion(asignacionData),
    onSuccess: (data) => {
  ...
  const updateAsignacionMutation = useMutation({
    mutationFn: ({ idAsignacion, updateData }) =>
      demoAsignacionAulaService.updateAsignacion(idAsignacion, updateData),
    onSuccess: () => {
  ...
  const deleteAsignacionMutation = useMutation({
    mutationFn: (idAsignacion) =>
      demoAsignacionAulaService.deleteAsignacion(idAsignacion),
    onSuccess: () => {
  ...
  const getAsignacionesByTrabajador = (idTrabajador) => {
    return useQuery({
      ...
      queryFn: () =>
        demoAsignacionAulaService.getAsignacionesByTrabajador(idTrabajador),
  ...
  const fetchAulasPorGrado = useCallback(
    async (idGrado) => {
      try {
        ...
        // Usar el servicio DEMO para obtener aulas disponibles por grado
        const aulasDisponibles =
          await demoAulaService.getAulasDisponiblesPorGrado(idGrado);
