...
export const aprobarPlanillasMasivo = async (data) => {
  ...
  return {
    success: true,
    message:
      "Esta es una demostración. Para aprobar planillas reales, adquiere el software completo.",
  };
};
...
export const getEstadisticas = async () => {
  ...
  const stats = {
    ...
    pendientes: planillasMensuales.filter(
      (p) => p.estadoPlanilla === "PENDIENTE"
    ).length,
    aprobadas: planillasMensuales.filter((p) => p.estadoPlanilla === "APROBADA")
      .length,
    pagadas: planillasMensuales.filter((p) => p.estadoPlanilla === "PAGADA")
      .length,
    generadas: planillasMensuales.filter((p) => p.estadoPlanilla === "GENERADA")
      .length,
    rechazadas: planillasMensuales.filter(
