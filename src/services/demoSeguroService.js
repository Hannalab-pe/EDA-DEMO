...
export const getTipoSeguroById = async (id) => {
  ...
  const seguro = mockData.tiposSeguros.find(
    (s) => s.idTipoSeguro === parseInt(id)
  );
...
export const createTipoSeguro = async (data) => {
  ...
  const nuevoSeguro = {
    ...
    porcentajeDescuento:
      data.tipoCalculo === "PORCENTAJE"
        ? parseFloat(data.porcentajeDescuento)
        : null,
    montoFijo:
      data.tipoCalculo === "MONTO_FIJO" ? parseFloat(data.montoFijo) : null,
...
export const updateTipoSeguro = async (id, data) => {
  ...
  const index = mockData.tiposSeguros.findIndex(
    (s) => s.idTipoSeguro === parseInt(id)
  );
...
export const toggleEstadoSeguro = async (id) => {
  ...
  mockData.tiposSeguros[index].estaActivo =
    !mockData.tiposSeguros[index].estaActivo;
...
export const getEstadisticas = async () => {
  ...
  const segurosPorcentaje = seguros.filter(
    (s) => s.tipoCalculo === "PORCENTAJE"
  ).length;
  const segurosMontoFijo = seguros.filter(
    (s) => s.tipoCalculo === "MONTO_FIJO"
  ).length;
