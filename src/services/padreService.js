...
import { padresDemoService, DEMO_MODE } from './demoService';
...
export const padreService = {
  ...
  async getAllParents(filters = {}) {
    if (DEMO_MODE) {
      try {
        const result = await padresDemoService.getAll(filters);
        return result.data;
      } catch (error) {
        console.error('🎭 Demo Error:', error.message);
        throw new Error('Error al obtener padres en modo demo');
      }
    }
