...
const TareaCompletaModal = ({ isOpen, onClose, tarea }) => {
  ...
  const cargarEntregas = async () => {
    ...
    try {
      ...
      // Si no hay entregas en la tarea, usar datos demo
      if (!entregasData.length && tarea.idTarea) {
        console.log("🎭 Demo: Generando entregas simuladas para la tarea...");
        ...
        const entregasDemo = [
          {
            idTareaEntrega: 1,
            estudianteId: "1",
            estudiante: { nombre: "Sofía", apellido: "Pérez", foto: "👧" },
            realizoTarea: true,
            fechaEntrega: new Date().toISOString(),
            observaciones: "Excelente trabajo realizado",
            archivoUrl: null,
          },
          {
            idTareaEntrega: 2,
            estudianteId: "2",
            estudiante: { nombre: "Carlos", apellido: "Mendoza", foto: "👦" },
            realizoTarea: false,
            fechaEntrega: null,
        ...
        console.log("📋 Entregas demo generadas:", entregasDemo);

        // Filtrar entregas demo
        const realizadas = entregasDemo.filter(
          (entrega) => entrega.realizoTarea === true
        );
        const pendientes = entregasDemo.filter(
          (entrega) => entrega.realizoTarea === false
        );
        ...
        const realizadas = entregasData.filter(
          (entrega) =>
            entrega.realizoTarea === true || entrega.estado === "entregado"
        );
        const pendientes = entregasData.filter(
          (entrega) =>
  ...
  const totalEstudiantes =
    entregasRealizadas.length + entregasPendientes.length;
  ...
  if (
    isOpen &&
    (entregasRealizadas.length > 0 || entregasPendientes.length > 0)
  ) {
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-green-100">
                        <div className="flex items-center space-x-2">
                          ...
                          <span>
                            {tarea?.aula?.grado || "Sin grado"} -{" "}
                            {tarea?.aula?.seccion || "Sin sección"}
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                                  ...
                                  <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-xs text-gray-600">
                                        Archivo adjunto por el profesor
                                      </p>
                                    </div>
                                  </div>
                        ...
                        <div className="space-y-4">
                          {loading ? (
                            <div className="flex flex-col items-center justify-center py-12">
                              ...
                              <p className="text-gray-600">
                                Obteniendo información de las entregas
                                realizadas
                              </p>
                            </div>
                                    ...
                                    <div className="flex items-center space-x-3">
                                      <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center">
                                        <span className="text-green-600 font-semibold text-sm">
                                          {entrega.estudiante?.nombre?.charAt(
                                            0
                                          )}
                                          {entrega.estudiante?.apellido?.charAt(
                                            0
                                          )}
                                        </span>
                                      </div>
                                      <div>
                                        <p className="font-medium text-gray-900">
                                          ...
                                          {entrega.estudiante?.apellido}
                                        </p>
                                        <p className="text-sm text-gray-600">
                                          DNI:{" "}
                                          {entrega.estudiante?.nroDocumento}
                                        </p>
                                      </div>
                                    </div>
                                  ...
                                  {entrega.archivoUrl && (
                                    <div className="mt-3 flex items-center justify-between bg-white rounded-lg p-3 border">
                                      ...
                                      <div className="flex space-x-2">
                                        <button
                                          onClick={() =>
                                            abrirArchivo(entrega.archivoUrl)
                                          }
                                          className="flex items-center space-x-1 px-3 py-1 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors text-xs"
                                        >
                                          <Eye className="w-3 h-3" />
                                        ...
                                        <button
                                          onClick={() =>
                                            descargarArchivo(
                                              entrega.archivoUrl,
                                              `entrega_${entrega.estudiante?.nombre}_${entrega.estudiante?.apellido}`
                                            )
                                          }
                                          className="flex items-center space-x-1 px-3 py-1 bg-green-100 text-green-700 rounded-md hover:bg-green-200 transition-colors text-xs"
                                        >
                            ...
                            <div className="bg-gray-50 p-4 rounded-lg">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <p className="mt-1 text-sm text-gray-900">
                                    {tarea.idTrabajador?.nombre}{" "}
                                    {tarea.idTrabajador?.apellido}
                                  </p>
                                </div>
                                <div>
                                  <label className="text-sm font-medium text-gray-500">
                                ...
                                <div>
                                  ...
                                  <p className="mt-1 text-sm text-gray-900">
                                    {tarea.idTrabajador?.idTrabajador ||
                                      "No especificado"}
                                  </p>
                                </div>
                              </div>
