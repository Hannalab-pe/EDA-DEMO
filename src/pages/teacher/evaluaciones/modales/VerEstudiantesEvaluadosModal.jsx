...
const VerEstudiantesEvaluadosModal = ({
  isOpen,
  onClose,
  idAula,
  evaluacion,
  ...
  const fetchLibretaCalificaciones = async () => {
    ...
    try {
      console.log(
        "🔍 Cargando libreta de calificaciones (DEMO) para aula:",
        idAula
      );
      ...
      // Obtener evaluaciones existentes para esos estudiantes
      const evaluacionesDelAula = mockData.evaluaciones.filter((evaluacion) =>
        estudiantesDelAula.some((est) => est.id === evaluacion.estudianteId)
      );

      const libretasCompletas = [];

      evaluacionesDelAula.forEach((evaluacion) => {
      ...
      const totalEvaluados = libretasCompletas.length;
      ...
      setEstadisticas({
        ...
        estadisticasGenerales: {
          promedio: "A-B",
          mejorEstudiante:
            totalEvaluados > 0 ? libretasCompletas[0].estudiante.nombre : "N/A",
        },
      });

      console.log(
        "✅ Datos cargados (DEMO):",
        libretasCompletas.length,
        "evaluaciones"
      );
    } catch (error) {
                ...
                >
                  <span className="flex items-center">
                    ...
                    <span className="text-sm sm:text-base">
                      Estudiantes Evaluados - Libreta de Calificaciones
                    </span>
                  </span>
                  <button
                    type="button"
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                        ...
                        <div className="text-center">
                          <div className="text-2xl font-bold text-green-600">
                            {estadisticas.estudiantesEvaluados}
                          </div>
                        ...
                        <div className="text-center">
                          <div className="text-2xl font-bold text-purple-600">
                            {estadisticas.estadisticasGenerales.totalLogrados}
                          </div>
                        ...
                        <div className="text-center">
                          <div className="text-2xl font-bold text-orange-600">
                            {estadisticas.estadisticasGenerales.totalEnProceso}
                          </div>
                    ...
                    {isLoading ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-600">
                          Cargando libreta de calificaciones...
                        </p>
                      </div>
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-600">
                          No hay estudiantes evaluados aún
                        </p>
                      </div>
                    ) : (
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3">
                            ...
                            <span className="text-xs sm:text-sm text-gray-500 bg-gray-100 px-2 py-1 rounded">
                              {item.estudiante.tipoDocumento}:{" "}
                              {item.estudiante.nroDocumento}
                            </span>
                          </div>
                          ...
                          <div className="space-y-3">
                            {item.libreta.areas.map((area, areaIndex) => (
                              <div
                                key={areaIndex}
                                className="bg-gray-50 p-2 sm:p-3 rounded-md border-l-4 border-blue-500"
                              >
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2">
                                  <span className="font-medium text-gray-700 text-sm sm:text-base mb-1 sm:mb-0">
                                    {area.area}
                                  </span>
                                  <span
                                    className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium border ${getCalificacionColor(
                                      area.calificacion
                                    )}`}
                                  >
                                    {area.calificacion}
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-1 sm:gap-2 text-xs sm:text-sm text-gray-600 mb-2">
                                  ...
                                  <div>
                                    ...
                                    <span
                                      className={`ml-1 px-1 sm:px-2 py-0.5 rounded text-xs ${
                                        area.nivelLogro === "Logro esperado"
                                          ? "bg-green-100 text-green-800"
                                          : area.nivelLogro === "En proceso"
                                  ...
                                  area.evaluaciones.length > 0 && (
                                    <div className="bg-white p-2 rounded border text-xs sm:text-sm">
                                      ...
                                      <div className="text-gray-600">
                                        ...
                                        {area.evaluaciones[0].descripcion}
                                      </div>
