...
import {
  ...
  useEstudiantesAulaDemo,
  useAsistenciasPorAulaYFechaDemo,
} from "../../../hooks/demo/useAsistenciasDemo";
...
const Asistencia = () => {
  ...
  useEffect(() => {
    if (asistenciasRegistradas.length > 0) {
      ...
      console.log(
        "🔍 DEBUG - Estructura de primera asistencia:",
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "📊 Asistencias registradas disponibles:",
        asistenciasRegistradas.length
      );
      ...
      estudiantes.forEach((estudiante) => {
        ...
        if (asistenciaExistente) {
          console.log(
            "✅ Asistencia encontrada para estudiante",
            idEstudiante,
            ":",
            asistenciaExistente
          );
  ...
  useEffect(() => {
    if (selectedAula && selectedDate) {
      console.log(
        "🔄 Refrescando asistencias para nueva fecha/aula:",
        selectedDate,
        getAulaId(selectedAula)
      );
      refetchAsistenciasExistentes();
    ...
    setAsistencias((prev) => ({
      ...
      [estudianteId]: {
        ...
        observaciones:
          observaciones !== null
            ? observaciones
            : prev[estudianteId]?.observaciones || "",
      },
    }));
  };
  ...
  const handleGuardarAsistencias = async () => {
    ...
    try {
      ...
      toast.success(
        `Asistencias guardadas correctamente para ${asistenciasData.asistencias.length} estudiantes`
      );
  ...
  const estudiantesFiltrados = estudiantes.filter((estudiante) => {
    const nombreCompleto = `${estudiante.nombres || estudiante.nombre || ""} ${
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm text-red-600">
            {errorEstudiantes?.message ||
              errorEstudiantes?.toString() ||
              "Error desconocido"}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Error al cargar las asistencias existentes
          </p>
          <p className="text-sm text-red-600">
            {errorAsistenciasExistentes?.message ||
              errorAsistenciasExistentes?.toString() ||
              "Error desconocido"}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-2">
            Selecciona un aula para continuar
          </p>
          <p className="text-sm text-gray-500">
            Elige un aula del selector arriba
          </p>
        </div>
      </div>
    );
            ...
            {loadingEstudiantes || loadingAsistenciasExistentes ? (
              ...
              <div className="grid gap-4">
                {estudiantesFiltrados.map((estudiante, index) => {
                  const idEstudiante =
                    estudiante.id_estudiante || estudiante.idEstudiante;
                  const asistenciaActual = asistencias[idEstudiante] || {
                    estado: "",
                    observaciones: "",
                  };
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        {asistenciaActual.estado && (
                          <div
                            className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${getEstadoColor(
                              asistenciaActual.estado
                            )}`}
                          >
