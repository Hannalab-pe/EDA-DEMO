...
const Cronogramas = () => {
  ...
  const aulas = Array.isArray(aulasData)
    ? aulasData
    : aulasData?.data
    ? aulasData.data
    : aulasData?.info?.data
  ...
  const handleSaveActivity = async (activityData) => {
    try {
      console.log("💾 [DEMO] Guardando actividad:", activityData);

      // Solo agregar el ID del trabajador si no está ya presente
      const activityWithUser = {
        ...activityData,
        ...(activityData.idTrabajador
          ? {}
          : { idTrabajador: currentUser?.id || currentUser?.idUsuario || "" }),
      };

      console.log("📤 [DEMO] Datos finales a enviar:", activityWithUser);

      // Crear la actividad usando el servicio demo
      await demoCronogramaService.create(activityWithUser);
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Vista general de actividades por aula
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
              ...
              <div>
                <div className="font-medium text-blue-900">
                  {selectedAula.seccion}
                </div>
                <div className="text-sm text-blue-700">
                  {selectedAula.cantidadEstudiantes} estudiantes
                </div>
              </div>
            </div>
          )}
        ...
        {!selectedAula && !loadingAulas && (
          <div className="mt-4 p-4 bg-gray-50 rounded-lg text-center">
            ...
            <p className="text-gray-600">
              Selecciona un aula para ver su cronograma
            </p>
          </div>
        )}
        ...
        {errorAulas && (
          <div className="mt-4 p-4 bg-red-50 rounded-lg">
            <p className="text-red-600">
              Error al cargar aulas: {errorAulas.message}
            </p>
          </div>
        )}
      </div>
