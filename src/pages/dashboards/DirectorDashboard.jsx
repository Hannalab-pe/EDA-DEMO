        ...
        <div className="mt-auto p-3 border-t border-gray-200">
          <div className="flex flex-row items-center bg-gray-200 rounded-xl px-3 py-2 mb-3 w-full shadow gap-3 hover:-translate-y-1 transition-all hover:bg-blue-100 cursor-pointer">
            ...
            <div className="flex flex-col min-w-0">
              ...
              {user?.role?.nombre && (
                <span className="text-[10px] text-white bg-blue-600 rounded px-2 py-0.5 mt-1 mb-1 w-fit font-semibold tracking-wide uppercase">{user.role.nombre}</span>
              )}
            </div>
          </div>
          <button className="w-full flex items-center space-x-3 px-4 py-3 text-red-600 hover:bg-red-50 rounded-lg transition-colors duration-200" onClick={logout}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden lg:ml-0">
        <header className="bg-blue-600 border-gray-200 px-4 lg:px-6 py-4">
          <div className="flex items-center justify-between">
            <button className="lg:hidden p-2 text-gray-400 hover:text-gray-600" onClick={() => setIsMobileMenuOpen(true)}>
