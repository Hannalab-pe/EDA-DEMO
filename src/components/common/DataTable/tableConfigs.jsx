...
export const gradosColumns = [
  ...
  },
  {
    ...
    Cell: ({ value, row }) => (
      <div className="text-sm text-gray-900">
        {value && row.original?.edadMaxima
          ? `${value} - ${row.original.edadMaxima} años`
          : "No especificado"}
      </div>
    ),
  },
  {
    ...
    Cell: ({ value }) => (
      <div className="text-sm text-gray-900">
        {value ? `${value} estudiantes` : "No especificado"}
      </div>
    ),
  },
  {
    ...
    Cell: ({ value }) => (
      <span
        className={`text-sm font-medium ${
          value > 0 ? "text-red-600" : "text-gray-500"
        }`}
      >
        {value || 0}
      </span>
...
export const trabajadoresColumns = [
  ...
  },
  {
    ...
    Cell: ({ value }) => {
      if (!value || !Array.isArray(value) || value.length === 0) {
        return (
          <span className="text-sm text-gray-400 italic">Sin contrato</span>
        );
      }
      ...
      return (
        <span
          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getContractTypeBadgeColor(
            tipoContrato
          )}`}
        >
  ...
  },
  {
    ...
    Cell: ({ value }) => {
      if (!value || !Array.isArray(value) || value.length === 0) {
        return <span className="text-sm text-gray-400 italic">Sin cargo</span>;
      }
      ...
      return (
        <div>
          <div className="text-sm font-medium text-gray-900">
            {contrato.cargoContrato || "Sin cargo"}
          </div>
          <div className="text-xs text-gray-500">
            {contrato.lugarTrabajo || "Sin lugar"}
          </div>
        </div>
      );
    },
  },
  {
    ...
    Cell: ({ value }) => {
      if (!value || !Array.isArray(value) || value.length === 0) {
        return <span className="text-sm text-gray-400 italic">Sin sueldo</span>;
      }
      ...
      return (
        <div className="text-sm font-medium text-gray-900">
          S/{" "}
          {parseFloat(sueldo).toLocaleString("es-PE", {
  ...
  },
  {
    ...
    Cell: ({ value }) => {
      ...
      return (
        <span
          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getContractStatusBadgeColor(
            estado
          )}`}
        >
...
export const aulasColumns = [
  ...
  },
  {
    ...
    sortable: true,
    Cell: ({ value }) => (
      <span className="text-sm text-gray-900">{value || "Sin ubicación"}</span>
    ),
  },
  {
    Header: "Equipamiento",
...
export const asignacionCursosColumns = [
    ...
    Cell: ({ value }) => (
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <div className="text-sm text-gray-500">
            DNI: {value?.nroDocumento || "Sin documento"}
          </div>
        </div>
      </div>
    ),
  },
  {
    ...
    Cell: ({ value }) => (
      <div>
        ...
        <div className="text-sm text-gray-500">
          Código: {value?.codigo || "Sin código"}
        </div>
      </div>
    ),
  },
  {
    ...
    Cell: ({ value, row }) => (
      <div>
        ...
        <div className="text-sm text-gray-500">
          Sección: {row.original?.idAula?.seccion || "Sin sección"}
        </div>
      </div>
    ),
  },
  {
    ...
    sortable: true,
    Cell: ({ value }) => (
      <div className="text-sm text-gray-900">{value || "Sin año"}</div>
    ),
  },
  {
    Header: "Estado",
...
export const cursosColumns = [
  ...
  {
    Header: "Horas/Semana",
    accessor: "horasPorSemana",
    sortable: true,
    Cell: ({ value }) => (
      <div className="text-sm text-gray-900 font-medium">{value || 0} hrs</div>
    ),
  },
  {
    Header: "Estado",
